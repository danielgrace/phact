...
class Config
{
    ...
    public function __construct($config, $mode = self::PARSE_DOTS)
    {
        ...
        $env = (isset($config[$host])) ? $config[$host] : 'DEV';

        $data = array_merge($config['GLOBAL'], $config[$env]);

        $data['env'] = $env;
        $data['request.host'] = $host;
        $data['request.ip'] = $_SERVER['REMOTE_ADDR'];
        $data['request.uri'] = $_SERVER['REQUEST_URI'];

        $data['request.ua'] = isset($_SERVER['HTTP_USER_AGENT'])
                            ? $_SERVER['HTTP_USER_AGENT']
                            : 'Unknown';

        $data['request.method'] = $_SERVER['REQUEST_METHOD'];
        $data['request.query'] = $_SERVER['QUERY_STRING'];
        $data['request.referer'] = isset($_SERVER['HTTP_REFERER'])
