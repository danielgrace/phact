...
@interface CollectionViewHelper ()
{
...
- (void)handleLongPressGesture:(UILongPressGestureRecognizer *)sender
{
    ...
    switch (sender.state) {
        ...
        case UIGestureRecognizerStateCancelled: {
            ...
            BOOL moveLastItem = [(id<UICollectionViewDataSource_Draggable>)self.collectionView.dataSource collectionView:self.collectionView
                                                                              moveItemAtIndexPath:self.layoutHelper.fromIndexPath
                                                                                      toIndexPath:self.layoutHelper.toIndexPath];
            
            NSIndexPath *indexPathToRemove = [NSIndexPath indexPathForItem:2
                                                                 inSection:0];
            
            
            NSIndexPath *indexPathToAdd = [NSIndexPath indexPathForItem:0
                                                              inSection:1];

            NSLog(@"%@, %@",indexPathToRemove,indexPathToAdd );
            ...
            [self.collectionView performBatchUpdates:^{
                ...
                if(moveLastItem)
                {
                    [self.collectionView moveItemAtIndexPath:indexPathToRemove toIndexPath:indexPathToAdd];
                }
